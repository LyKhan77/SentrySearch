...
export function SearchResultsGrid({ results, onSelectClip }: { results: Clip[]; onSelectClip?: (clip: Clip) => void }) {
  if (!results.length) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {results.map((result) => (
        <ClipCard key={result.id} clip={result} onClick={() => onSelectClip?.(result)} />
      ))}
    </div>
  );
}
